...
function ProcessDispatch(powam, charam, count)
    ...
    for rname in region_names
        ...
        if create_plot == true
            ...
            pyimportjulia.CreateDisStack(dispatchpath, count, filename, filename_charge)
        else
            nothing
        end
    ...
    CSV.write(joinpath(dispatchpath, string(count), "dispatch_summary.csv"), dispatch_summary)
    ...
    if create_plot == true
        for rname in region_names
            ...
            CreateBarLine(isbarplot = true,
                          ...
                          leglab = nothing,
                          rpath = joinpath(dispatchpath, string(count)))
        end
    else
        nothing
    end
