generators = CSV.read(joinpath(inputpath_main, "Generators_data_" * deratepercent * ".csv"), DataFrame) # Reading generator data and create a data frame
demand = CSV.read(joinpath(inputpath_main, "Load_data_" * loadgrowth * ".csv"), DataFrame)
...
# Read path for scenarios for generators' variability
genvar = CSV.read(joinpath(inputpath_main, "Generators_variability_" * deratepercent * "_" * weatheryear * ".csv"), DataFrame) # Reading generator variability data and create a data frame
...
# Get a clearer version of demand data frame 
...
# Get a clearer version of heat time data frame 
heattime = select(heattime, r"^Heat_time_z")

numweek = Int(first(size(load)) / hours_per_period) 

# Set weight for each week if running annual model
