    ...
    for (count, (load, genvar, mlt)) in enumerate(zip(load_transition, genvar_transition, mlt_transition))
        ...
        for g in setCHP
            ...
            if  generators[generators.R_ID .== g, :Cap_Size][1] == 200
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 155)   
            elseif generators[generators.R_ID .== g, :Cap_Size][1] == 300
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 241)  
            elseif generators[generators.R_ID .== g, :Cap_Size][1] == 330
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 279)   
            elseif generators[generators.R_ID .== g, :Cap_Size][1] == 600
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 410)   
            else
                nothing
            end
        end
