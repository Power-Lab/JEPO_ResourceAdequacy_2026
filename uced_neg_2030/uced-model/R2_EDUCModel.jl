...
function EDUCModel()
    for (count, (load, genvar, mlt)) in enumerate(zip(load_transition, genvar_transition, mlt_transition))
        ...
        # VARIABLES
        # Unit commitment variables; Interger (!!! Running in TSCC cluster), using the lines below:
        ...
        # # In case want to change to linear (!!! Running in local laptop), using the lines below:
