...
# Generator variability with price-cap-driven derates
if scenario_name ∉ ["FlexibleSpotMLT"]
    genvar = CSV.read(joinpath(inputpath_main, "Generators_variability_derate35%.csv"), DataFrame)
...
hours_per_period = Int(first(other_info[other_info.Parameter .== "hours_per_period", :Value])) # hours per period
...
path_names = network_fwd.transmission_path_name # transmission path names

# Extract load columns by zone
...
# NSE cost is higher in NC than NE
