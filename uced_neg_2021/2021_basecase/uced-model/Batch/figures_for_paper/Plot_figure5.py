...
# Technology color mapping reused across plots
tech_colors = {
...
def process_scenario(base_path, scenario, week_number, ax, tech_order=None):
    """Process a single scenario and plot onto the provided axis."""
    ...
    df['week'] = df['period'].str.extract(r'week (\d+)').astype(int)
    ...
    ax.plot(demand_df.index, demand_df.values, color='black', linestyle='--', label='Demand')

    ax.set_ylim(bottom=-5000)
    ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f'{int(x):,}'))
    ax.yaxis.grid(True, linestyle='--', linewidth=0.5, color='grey', alpha=0.4)

    ax.set_xlim(left=0, right=169)
    ax.set_xticks([0] + list(range(24, 169, 24)))
    ax.set_xticklabels([str(x) for x in [0] + list(range(24, 169, 24))])

    ax.set_title(f"{scenario}", fontsize=24)
    ax.set_xlabel("Hour", fontsize=14)
    ax.set_ylabel("Dispatch and Demand (MW)", fontsize=14)
...
for ax, scenario in zip(axes_flat, scenario_list):
    plotted = process_scenario(base_path, scenario, week_number, ax)
    if not plotted:
        ax.set_title(f"{scenario} (no data)", fontsize=16)
        ax.axis('off')
...
labels = ['Demand']
handles.extend([
    plt.Line2D([0], [0], color=tech_colors[key], lw=4)
    for key in tech_colors
    if key != 'Demand'
])
labels.extend([label_mapping.get(key, key.capitalize()) for key in tech_colors if key != 'Demand'])
...
fig.savefig('figure5.pdf', dpi=300, bbox_inches='tight')
plt.close(fig)
