...
for (path in Results_path){
  ...
  temp_plot_long <- ggplot() +
    ...
    scale_linewidth_continuous(range = c(0.5, 9), guide = "none") +  # Increase the contrast in line thickness
    ...
    geom_text(data = NE_long_lat, aes(x = lon, y = lat, label = zone),
          color = "black", size = 10, nudge_x = -1.2, nudge_y = 1.2) +
...
ggsave(file.path('figure4.pdf'), combined_plot, width = 16, height = 16)