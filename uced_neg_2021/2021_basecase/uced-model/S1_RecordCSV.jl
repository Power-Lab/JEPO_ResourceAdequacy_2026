function RecordCSV(unitc, flowfwd, flowrvs, flowcombine, unmetdec, allcap, genvar, count)
    ...
    colnames = [:R_ID, :STOR, :SOLAR, :COAL, :WIND, :GAS, :HYDRO, :NUCLEAR, :NONDISP] # Common columns

    allcap_res = innerjoin(allcap_data, generators[!, colnames], on = :R_ID) 
    ...
    sort!(allcap_res, [:R_ID])
        ...
        final_curtailment_wind.TotalPotentials = total_potentials
        final_curtailment_wind.TotalActuals = total_actuals
        ...
        final_curtailment_solar.TotalPotentials = total_potentials
        final_curtailment_solar.TotalActuals = total_actuals
