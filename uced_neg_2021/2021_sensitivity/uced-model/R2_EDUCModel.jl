...
function EDUCModel()
    for (count, (load, genvar, mlt)) in enumerate(zip(load_transition, genvar_transition, mlt_transition))
        ...
        @variable(EDUC_Model, vCOMMIT[setUC, setTIME],  Bin) # Committed UC units
        @variable(EDUC_Model, vSTARTUC[setUC, setTIME], Bin) # UC starts
        @variable(EDUC_Model, vSHUTUC[setUC, setTIME],  Bin) # UC shutdowns
        ...
        for g in setCHP
            ...
            if  generators[generators.R_ID .== g, :Cap_Size][1] == 200
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 155)   # 200 -> 155
            elseif generators[generators.R_ID .== g, :Cap_Size][1] == 300
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 241)   # 300 -> 241
            elseif generators[generators.R_ID .== g, :Cap_Size][1] == 330
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 279)   # 330 -> 279
            elseif generators[generators.R_ID .== g, :Cap_Size][1] == 600
                @constraint(EDUC_Model, [t in setTIME], vGENDISPATCH[g,t] <= genvar[t,g] * vCOMMIT[g,t] * 410)   # 600 -> 410
            else
                nothing
            end
        end

        # Generation dispatch <= available capacity
        ...
        # Generation dispatch >= minimum output
        ...
        # Hydro SOC balance (inflow assumed as vCAPGEN * genvar)
        ...
        for l in temp_setLINE
            ...
            for t in setTIME[1:end]
                if mlt[t,line_name] >= 0
                    @constraint(EDUC_Model, vFLOWFWD[l, t] <= 1.1*mlt[t,line_name] * (1-loss_rate/2)) 
                    @constraint(EDUC_Model, vFLOWFWD[l, t] >= 0.9*mlt[t,line_name] * (1-loss_rate/2)) 
                    @constraint(EDUC_Model, vFLOWRVS[l, t] == 0)
                else
                    @constraint(EDUC_Model, vFLOWFWD[l, t] == 0)
                    @constraint(EDUC_Model, vFLOWRVS[l, t] <= 1.1*abs(mlt[t,line_name] * (1-loss_rate/2))) 
                    @constraint(EDUC_Model, vFLOWRVS[l, t] >= 0.9*abs(mlt[t,line_name] * (1-loss_rate/2))) 
                end
            end
        end
        ...
        # Objective = variable cost + NSE + starts
